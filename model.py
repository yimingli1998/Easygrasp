...
class backbone_pointnet2(nn.Module):
    def __init__(self):
        ...
        if cfg['dataset']['use_norm_points']:
            self.sa1 = PointnetSAModule(mlp=[3+3, 32, 32, 64], npoint=1024, radius=0.1, nsample=32, bn=True)
        else:
            self.sa1 = PointnetSAModule(mlp=[0+3, 32, 32, 64], npoint=1024, radius=0.1, nsample=32, bn=True)
        ...
        if cfg['dataset']['use_norm_points']:
            self.fp1 = PointnetFPModule(mlp=[128+6+3, 128, 128])
        else:
            self.fp1 = PointnetFPModule(mlp=[128+3+3, 128, 128])
        ...
        if cfg['train']['use_bin_loss']:
            ...
            self.channels += per_width_bin_num * 2
            self.channels += per_depth_bin_num * 2
            self.channels += per_score_bin_num * 2
        else:
            self.channels = 2+4+3
        self.conv3 = nn.Conv1d(128, self.channels, kernel_size=1, bias=False)
    ...
    def forward(self, xyz, points):
        ...
        bat_pred = self.conv3(feature).permute(0,2,1).contiguous()   # B,N,C,5

        pred_graspable, pred_grasp = bat_pred[:,:,:2],bat_pred[:,:,2:]
        # print(pred_graspable.shape, pred_grasp.shape)
        ...
        return pred_graspable, pred_grasp

    def get_bin_loss(self,pred,data):
        bat_pred_graspable,bat_pred_grasp = pred
        graspable_label = data['grasp_point_label']
        grasp_label = data['grasp_label']
        if_gp_mask = torch.where(graspable_label > -1)
        gp_mask = torch.where(graspable_label > 0)
        ...
        gp_loss = nn.CrossEntropyLoss(self.gp_weight)(bat_pred_graspable[if_gp_mask],graspable_label[if_gp_mask].long())
        # print(pred_graspable.size())
        out_gp = torch.argmax(bat_pred_graspable, dim=2)
        # print(out_gp.size())
        # print(torch.sum(out_gp))
        # print(torch.sum(out_gp[if_gp_mask]))
        # exit()
        gp_acc = self.two_class_acc(out_gp[if_gp_mask],graspable_label[if_gp_mask])
        acc_dict = {
            'TP':        gp_acc[0],
            'FP':        gp_acc[1],
            'TN':        gp_acc[2],
        ...
        bin_loss_dict, grasp_loss = loss_utils.get_bin_reg_loss(bat_pred_grasp[gp_mask],grasp_label[gp_mask])
        loss_dict = {
            'gp_loss':          gp_loss,
            'azimuth_loss':     bin_loss_dict['azimuth_bin_loss']+bin_loss_dict['azimuth_res_loss'],
            'elevation_loss':    bin_loss_dict['elevation_bin_loss']+bin_loss_dict['elevation_res_loss'],
            'grasp_angle_loss':  bin_loss_dict['grasp_angle_bin_loss']+bin_loss_dict['grasp_angle_res_loss'],
            'width_loss':        bin_loss_dict['width_bin_loss']+bin_loss_dict['width_res_loss'],
            'depth_loss':        bin_loss_dict['depth_bin_loss']+bin_loss_dict['depth_res_loss'],
            'score_loss':        bin_loss_dict['score_bin_loss']+bin_loss_dict['score_res_loss'],
            'total_loss':       gp_loss + grasp_loss
#             'total_loss':       gp_loss
        }
    ...
    #     gt_list = []
    #     tax_list = []
    #     bat_pred_graspable,bat_pred_depth,bat_pred_quat,bat_pred_joint = pred
    #     for k in data.keys():
    #         if data[k].size(-1) == 27: # graspable 1, depth 1,quat 4 ,metric 1 ,joint 20
    #             gt_list.append(data[k])
    #             tax_list.append(k)
    #     assert len(gt_list) == bat_pred_quat.size(-1)
    #     loss_dict, acc_dict = {},{}
    #     for i,gt in enumerate(gt_list):
    #         graspable, depth, quat, joint = gt[:,:,0].long(), gt[:,:,1], gt[:,:,2:6],gt[:,:,7:]
    #         pred_graspable,pred_depth,pred_quat = bat_pred_graspable[:,:,:,i],bat_pred_depth[:,:,i],bat_pred_quat[:,:,:,i]
    ...
    #
    #         depth_loss = nn.SmoothL1Loss()(pred_depth[gp_mask],depth[gp_mask])*100
    ...
    #             'rot_loss':       rot_loss,
    #             'loss':           gp_loss + depth_loss + rot_loss
    #         }
    #         acc_dict[tax_list[i]] = {
    #             'TP':        gp_acc[0],
    #             'FP':        gp_acc[1],
