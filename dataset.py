...
class SimGraspDataset:
    ...
    def __getitem__(self, index):
        ...
        grasp = np.load(os.path.join(BASE_DIR,f'scene_grasps/scene_grasp_{str(scene_id).zfill(4)}.npy'),allow_pickle =True).item()
...
if  __name__ =='__main__':
    data_path = os.path.join(BASE_DIR,'point_grasp_data_2mm')
    gd = SimGraspDataset(data_path)
    dataloader = torch.utils.data.DataLoader(gd, batch_size=2, shuffle=True,
                                             num_workers=16)
    t0 = time.time()
