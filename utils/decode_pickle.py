...
mesh_prefix_dir = '/data00/dlr/BlenderProc/scene_dataset/dataset/lm/models/'
device = 'cpu'
...
file_path = '/data00/dlr/pickle_dlr/'
        ...
        if filename.endswith('_final.pickle'):
            ...
            with open(filepath, 'rb') as f:
                ...
                for grasp_dict in grasp_dicts:
                    ...
                    print('offset:',norm_offset)
                    print('R:',R)
                    print(np.dot(R_hand[:3,:3],R))
                    print(np.dot(np.linalg.inv(R_hand[:3,:3]),R))
                    exit()
