...
if __name__ == '__main__':
    ...
    train_simgrasp = False
    if train_simgrasp:
        dataset_path = '../point_grasp_data_2mm/'
        train_data = SimGraspDataset(dataset_path,split='train')
        ...
        dataset_path = '../'
        train_data = GraspNetDataset(dataset_path,split='train')
        train_dataloader = torch.utils.data.DataLoader(train_data,
                                                       batch_size = FLAGS.batchsize,
                                                       shuffle=True,
                                                       num_workers = FLAGS.workers)
        test_data = GraspNetDataset(dataset_path,split='test_similar')
        test_dataloader = torch.utils.data.DataLoader(test_data,
                                                   batch_size = FLAGS.batchsize,
                                                   shuffle=True,
                                                   num_workers = FLAGS.workers)
